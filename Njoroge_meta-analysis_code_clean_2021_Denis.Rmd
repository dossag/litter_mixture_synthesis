...
knitr::opts_chunk$set(echo = TRUE, include=TRUE, message=FALSE, results="hide")
getwd()
```
...
This R script records Njoroge et al 2021 meta analysis on effect of fauna and litter mixture on litter decomposition across the globe. The manuscript is entitled "Soil fauna accelerate litter mixture decomposition globally, especially in dry environments" published in Journal of Ecology xxx.

This work is based on conceptual figure

![Hypothetical framework of this study](Figure 1 conceptual framework.png)

FIGURE 1 Hypothetical framework of this study. Litter mixture species richness and soil fauna are experimentally manipulated to depict their respective effect on litter decomposition (Hypotheses 1 and 2) and how precipitation levels influence those interactions. H1- Multiple species litter will have higher decomposition rate than the component single species litter in the mixture but this effect will soon saturate with an increase of litter species richness in mixture. H2- Inclusion of soil fauna will cause a change in the magnitude and direction of litter decomposition of multiple-species experiments, but not in single-species experiments. H3-With decreased precipitation, soil fauna will have a greater positive impact on the decomposition of litter mixtures, although we acknowledge a potential negative effect if dominant soil faunal taxa exert a net preferential feeding behaviour (Swan & Palmer, 2006b).
...
Add simple data points to map. We need to first know the unique points as studies has replicates.
...
Going back to the data exploration, some values are negative in m_expected and in m_observed. We confirmed with relevant authors about the accuracy of these data. The authors confirmed their accuracy, thus we deleted them from our next analysis.
...
Check with summary and see if the negative values were removed.
...
Save this funnel plot as supplementary material (Figure S1)
...
```
This number is greater  than the recommended threshold 5 k + 10 where k is our total number of observations (Rosenthal, 1979). Therefore, we can conclude that there is no publication bias.

## Statistical analysis (Hypothesis H1)
...
labels <- c("Present (274)", "Absent (593)", "Overall (867)")
forest(estimates, variances, slab=labels,col="black", header = "Fauna status",pch = 19, cex=2,xlab = "Effect size (ln(RR))", psize = 1.5)
```

We can also use ggplot2 package to plot the above forest plot. This gives more flexibility in graph plotting and leads to better visual impact (Figure 3) 
...
Plot the above forest plot using ggplot2 (Figure S4)
...
Plot the above forest plot using ggplot2 (Figure S2 a)
...
Plot precipitation and fauna with ggplot2 (Figure 4 a). 
...
Only the fauna status that shows significant effect is shown. This the Figure S3 in the supplementary material.
...
See the relationship of fauna status and incubation period (Figure S5).
...
#### Effect sizes based on the fauna status in different climatic regions
...
We used ggplot to plot the above plot (Figure 4 b)
...
Manually plot the above forest plots using ggplot since its easier to work on the visual impact of the plot (Figure 4 c).
```{r, results='hold', warning=FALSE}
dat10 <- data.frame(
...
#### Model for different ecosystems with fauna availability as the moderator
...
To visually look at the results, we use a forest plot
...
We also manually plot the above forest plots using ggplot2 (Figure S2 b).
...
We visually look at the results using forest plot.
...
We also manually plot the above forest plots using ggplot2 (Figure S2 c)
...
Metaregression for all the moderators together include the ones that are not highly correlated
...
We should not incorporate moderators that are highly correlated. Thus, we need to run correlation test.
...
Here we ensure that we only  keep  rows with no missing data from all predictors used. we used the metaregression results to choose the moderators to use in glmulti.
...
##### Plot of the AICc values for all 150 models
...
This study investigated litter decomposition which is an important process in nutrient cycling within ecosystems. The results add to the current ecology knowledge by revealing the averaged outcome of global decomposition of litter mixtures and their interactions between soil fauna effect and precipitation. 
